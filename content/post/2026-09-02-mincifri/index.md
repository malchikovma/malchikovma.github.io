...
image: mincifry-https.webp
draft: false
tags:
 - Руководство
description: Как получить бесплатный HTTPS сертификат от Минцифры для своего сайта на 365 дней. Смотрим, с какими сложностями предстоит столкнуться.
---
...
Идем на Госуслуги, вбиваем в поиске "SSL-сертификат", получаем тако диалог с Роботом Максом:
...
Я выбираю OV. Далее на выбор дают два варианта: RSA и ГОСТ. Я посчитал, что использование сертификатов Минцифры уже достаточно патриотично, поэтому выбрал RSA.
...
Сгенерируем CSR (Certificate Signing Request) на основе конфига:
...
Опционально, проверим валидность:
...
Но это конечные сертификаты, а для HTTPS нам нужна полная цепочка. [Скачиваем корневой и промежуточный (выпускающий) сертификат Минцифры.](https://www.gosuslugi.ru/crt). Находим среди них те, что в текстовом формате:
- для корневого сертификата - `russian_trusted_root_ca_pem.crt`,
- для промежуточного - `russian_trusted_sub_ca_pem.crt`.

Объединяем сертификаты в цепочку:
