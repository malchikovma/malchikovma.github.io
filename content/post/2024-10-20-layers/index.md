...
Сегодня поговорим про то, как построить приложение, состоящее больше чем их пары классов. Про архитектуру. В предыдущей статье мы обсуждали интерфейсы и как они позволяют изолировать классы друг от друга. Точно такой же подход можно применить к приложению в целом.

## Домен

Чем наше приложение отличается от сотен других? ~~Оно плохо написано.~~ Оно решает свою уникальную задачу. Настолько, что логику в ней, мы называем областью знаний. На английском это будет domain knowledge или просто домен. И по сравнению с ним, запросы к БД и общение с веб-сервером считаются уже решенной задачей. Их уже выполняют различные библиотеки и фреймворки.
...
В центре всего располагается домен, а в нем сущности (`Entity`), объекты значений (`ValueObject`), доменные сервисы (`DomainService`), репозитории (`Repository`). Такая терминология идет из Domain Driven Development (DDD) [^1] [^2]. В DDD область знаний мы ставим на первое место. Мы назваем объекты теми существительными, которые используют специалисты. Мы называем взаимодействие объектов глаголами, которые используют специалисты. Так как мы говорим с бизнесом на одном языке, нам проще понять друг друга. А разница между реальностью и кодом становится чуть меньше, что пораждает меньше путаницы и ошибок.
...
Раньше мы не встречались только с объектом значения, `ValueObject`. Он самый простой из своей компании. По сути означает одно или группу связанных значений, со своими ограничениями. Мы используем его вместо примитивов: встроенных типов языка программирования.
...
Ядро приложения не работает с внешним миром напрямую. Но ему приходится с ним взаимодейстовать. Как именно? Через интерфейсы. Но как называется, то место, где лежат их реализации? Инфраструктура.
...
У нас есть все необходимые ингридиенты, но нет рецепта. Кто определяет, как "приготовить" приложение, как собрать все воедино? Для этого нам понадобятся сервис уровня приложения, ApplicationService. В его методах мы опишем способы взаимодейтсвия с ядром.
...
{
    public function allocateUser(int $countryId, int $userId)
    {
...
Атомарность - это отдельная тема, сейчас не будем ее касаться. Только дадим неформальное определение: это попытка выполнить серию изменений как одно целое. В нашем примере мы сначала сохраняем пользователя, затем страну. Метод обернут в транзакцию, а значит в случае ошибки не случится такого, что сохранился пользователь, но не сохраниласть страна. Такую возможность нам обычно предоставляет база данных. Обеспечить атомарность среди разнородных сервисов значительно сложнее.
...
Если действий много, ApplicationService станет очень большим. В таком случае мы можем извлечь методы в отдельные классы. Такие классы мы назваем действиями или сценариями (UseCase):
...
У нас есть готовое приложение, но как с ним взаимодействовать? Если мы все сделали грамотно, то можем выбрать любой способ взаимодейтсвия. Давайте возьмем любой популярный web-фреймворк и запустим на нем наше приложение, например [Yii2 с шаблоном basic](https://github.com/yiisoft/yii2-app-basic). Не будем повторять инструкцию по установке, она уже есть на [официальном сайте Yii2](https://www.yiiframework.com/doc/guide/2.0/ru/start-installation).

Создадим модуль приложения. Не модуль фреймворка, а именно нашего собственного приложения. За основу возьмем пример со странами из [предыдущей статьи](/post/2024-10-18-oop-php). В composer.json укажем отдельный namespace, так как Yii2 еще не знает, как работать автозагрузкой по PSR-4:
...
Раскомментируем `prettyUrl` в файле конфигурации `config/web.php`. Запустим встроенный веб-свервер PHP в директории `web`: `php -S localhost:8080`. Постучимся к нашему приложению через эндпоинт "/country/density?name=japan". Про дизайн API поговорим в другой раз. А сейчас, видим, что все работает.
...
К сожалению, фреймворки хороши только для самых тривиальных задач: прочитать и обновить запись в БД. Если хоть немного отступить от официальной документации, начнется "темный лес". А их подход ограничивает нас: нужно все делать с оглядкой на задумку автора.
