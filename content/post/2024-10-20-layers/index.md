...
Сегодня поговорим про то, как построить приложение, состоящее больше чем из пары классов. Про архитектуру. В предыдущей статье мы обсуждали интерфейсы и как они позволяют изолировать классы друг от друга. Точно такой же подход можно применить к приложению в целом.
...
Раскомментируем `prettyUrl` в файле конфигурации `config/web.php`. Запустим встроенный веб-сервер PHP в директории `web`: `php -S localhost:8080`. Постучимся к нашему приложению через эндпоинт "/country/density?name=japan". Про дизайн API поговорим в другой раз. А сейчас, видим, что все работает.
